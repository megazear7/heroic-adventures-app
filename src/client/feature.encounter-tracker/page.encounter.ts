...
@customElement("page-encounter")
export class PageEncounter extends LitElement {
  static override styles = css`
    ...
    .round-history-modal {
      ...
      border-radius: 16px;
      box-shadow: var(--shadow-active);
      padding: var(--size-md) var(--size-md) 1.1rem;
    }
    ...
    @media (max-width: 600px) {
      ...
      .round-history-modal {
        ...
        border-bottom: none;
        padding: 0.9rem 0.85rem var(--size-md);
      }
    }
  `;
  ...
  private readonly onEncountersChanged = (): void => {
    ...
    // Refresh our local copy if it was changed externally
  ...
  override render() {
    if (!this.encounter) {
      return html`
        <a href="/encounters" class="back-link">${leftArrowIcon} Encounters</a>
        <div class="not-found">Encounter not found.</div>
      `;
    }
